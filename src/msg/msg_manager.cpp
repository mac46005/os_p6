...
MsgManager::MsgManager(const char *key, int permission, pid_t pid, Logger *logger): permission_(permission), pid_(pid), logger_(logger) {
    logger_->logDebugINFO("MsgManager::MsgManager()", "Initialized MsgManager()");
...
void MsgManager::sendMessage(long mtype, pid_t sender_pid, ProcessStatus status, int address, int page, int offset, AccessType access, int msg_flag) {
    ...
    logger_->logDebugWARNING("MsgManager::sendMessage()", "Message successfully sent to PID " + std::to_string(buf.mtype));

}

void MsgManager::recieveMessage(std::function<void(MsgBuffer)> message, int message_flag) {
    MsgBuffer buf;
    logger_->logDebugWARNING("MsgManager::recieveMessage()", "Attempting to msgrcv()");
    ...
    logger_->logDebugWARNING("MsgManager::recieveMessage()", "Message recieved from PID " + std::to_string(buf.sender_pid));
...
void MsgManager::cleanUp() {
    logger_->logDebugINFO("MsgManager::cleanUp()", "Cleaning up data");
    if (msqid_ == -1) {
        return;
    }
