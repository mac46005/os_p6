...
Clock::Clock(std::string parent, std::string key, Logger *logger): key_(key), parent_(parent), logger_(logger) {
    ...
    logger_->logDebugINFO("PID " + std::to_string(getpid()) + " Clock::Clock()","Intializing Clock");
...
void Clock::detach() {
    logger_->logDebugINFO("PID " + std::to_string(getpid()) + " Clock::detach()", "Detaching from clock");
    shmdt(current_time_);
}

void Clock::rmid() {
    logger_->logDebugINFO("PID " + std::to_string(getpid()) + " Clock::rmid()", "Removing shared memory in ipcs");
